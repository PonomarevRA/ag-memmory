...
function messageMarkup(message: ThreadMessage, memory?: readonly MemoryComponent[]): string {
  const memoryBlock = memory?.length
    ? `<ul class="message__memory">${memory.map(item => `<li><strong>${escapeHtml(item.title)}</strong><span class="scope-badge">${escapeHtml(item.type)}</span><p>${escapeHtml(item.preview)}</p></li>`).join('')}</ul>`
    : '';
  return `<article class="message message--${message.role}"><p class="message__label">${message.role === 'user' ? 'Вы' : 'Модель'}</p>${memoryBlock}<p class="message__content">${escapeHtml(message.content)}</p></article>`;
}
type MemoryComponent = { type: string; title: string; preview: string };
function wire(id: string, messages: ThreadMessage[]): void {
  ...
  form?.addEventListener('submit', event => {
    ...
    const memoryComponents: MemoryComponent[] = [];
    messages.push(user, assistant); input.value = ''; list.innerHTML = messages.map(message => messageMarkup(message)).join(''); setPending(true);
    const receiver: ChatReceiver = { invokeMethodAsync: async (method, item) => {
      if (method === 'OnChatEvent' && typeof item === 'object' && item) {
        const streamEvent = item as { kind?: number; text?: string; content?: string; errorCode?: string; memory?: MemoryComponent };
        if (streamEvent.kind === 2 && streamEvent.memory) memoryComponents.push(streamEvent.memory);
        if (streamEvent.kind === 0) {
          const chunk = typeof streamEvent.text === 'string' ? streamEvent.text : streamEvent.content;
          if (typeof chunk === 'string') assistant.content += chunk;
        }
        if (streamEvent.kind === 1) assistant.content = errorMessage(streamEvent.errorCode);
        list.innerHTML = messages.slice(0, -1).map(message => messageMarkup(message)).concat(messageMarkup(assistant, memoryComponents)).join('');
      }
