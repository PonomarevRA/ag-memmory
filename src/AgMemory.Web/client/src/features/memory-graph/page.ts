...
export async function memoryGraphPage(): Promise<string> {
  ...
  if (graph.status !== 'available') return `<section class="memory-graph-page"><header class="page-header"><p class="eyebrow">Структурная проекция</p><h1>Граф памяти</h1></header><div class="ui-status-banner ui-status-banner--warning"><h2>Граф недоступен</h2><p>Настройте локальное хранилище и scope на сервере.</p></div></section>`;
  graphSnapshot = graph;
