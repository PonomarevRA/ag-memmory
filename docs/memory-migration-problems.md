...
| P5-01 | Открыто | Schema manifest допускает только compatible pre-manifest registration; произвольная schema evolution не выполняется автоматически. | Автоматический upgrade мог бы повредить existing records. | Release owner выбирает explicit table-version/reindex migration workflow. | [phase-5-open-problems.md](phase-5-open-problems.md) |
| P6-01 | Открыто | Current budget asserts selected-memory token estimates; rendered transport/citation formatting needs a dedicated measurable tokenizer contract. | Product cannot yet claim a strict wire-format token ceiling. | Token-economy Phase 11 adds rendered-token estimator and tests. | [phase-6-open-problems.md](phase-6-open-problems.md) |
| P6-02 | Открыто | Vector/graph provider exceptions have safe fallback metadata, but production retry/telemetry/SLO policy is not yet configured. | Runtime operations remain incomplete. | Agent-integration/validation phases configure host telemetry and SLO gate. | [phase-6-open-problems.md](phase-6-open-problems.md) |
| P7-01 | Открыто | `RecentDecision` in hot state is deliberately short-lived context, not a durable decision audit. | It cannot fulfil reusable decision history alone. | Phase 8 adds compact durable decision trace and explicit promotion semantics. | [phase-7-open-problems.md](phase-7-open-problems.md) |
